...
class _AbstractUserge(Methods, RawClient):
    ...
    async def _load_plugins(self) -> None:
        _IMPORTED.clear()
        _INIT_TASKS.clear()
        logbot.edit_last_msg("Importing All Plug-ins", _LOG.info, _LOG_STR)
    ...
    async def reload_plugins(self) -> int:
        ...
        reloaded: List[str] = []
        _LOG.info(_LOG_STR, "Reloading All Plug-ins")
...
class UsergeBot(_AbstractUserge):
    ...
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting BOT Configs")
...
class Userge(_AbstractUserge):
    ...
    def __init__(self, **kwargs) -> None:
        _LOG.info(_LOG_STR, "Setting USERBOT Configs")
        kwargs = {
    ...
    async def start(self) -> None:
        ...
        await super().start()
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Starting USER-Bot🌠")
    ...
    async def stop(self) -> None:  # pylint: disable=arguments-differ
        """ stop client and bot """
        if self._bot is not None:
            _LOG.info(_LOG_STR, "Stopping USER-Bot 💤")
            await self._bot.stop()
        _LOG.info(_LOG_STR, "Stopping USER-X🔧")
        await super().stop()
        _close_db()
